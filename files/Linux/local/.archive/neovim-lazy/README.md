...
This is the **Neovim Lazy** catalog entry: snacks.nvim (LazyVim's default
explorer/picker) and neo-tree both render with a flush, non-elevated
background. The sibling `../neovim-nvf` directory is a separate catalog
entry — a frozen snapshot from before that change, where both still use the
elevated surface-container tone. Enable only one at a time in noctalia's
template settings; both write to the same output path.
