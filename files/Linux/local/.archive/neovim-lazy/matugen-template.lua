...
local function make_palette(mode)
  ...
  return {
    ...
    -- Tabline / bufferline chrome: the fill (gaps between tabs, empty tab
    -- area) is flush with the editor, inactive tabs sit one clear rung above
    -- it, and the selected tab sits higher still, so focus reads
    -- unambiguously even on a near-monochrome palette. Same roles in both
    -- modes -- the ramp runs darker with elevation in light and lighter in
    -- dark, so "higher rung" means "brighter" here since this template
    -- targets dark wallpapers most of the time.
    nc = base,
    tab_bg = m.surface_container,
    tab_sel = m.surface_container_highest,
...
local function make_lualine_theme(p)
  ...
  -- `b` steps down from the mode colour (`a`) rather than landing on a fixed
  -- M3 container tone -- surface_container_high (the old `p.hl_med`) isn't
  -- reliably between `a` and `c` in brightness, and could end up darker than
  -- the base itself. Blending the mode hue toward `base` guarantees `b`
  -- always sits between the two, and keeps a hint of the mode's colour. Text
  -- stays the plain editor fg rather than the same hue as the bg -- same hue
  -- at two opacities reads as one flat colour instead of actual contrast.
  local function section(hue, on_hue)
    return { a = { bg = hue, fg = on_hue, gui = 'bold' }, b = { bg = blend(hue, p.base, 0.3), fg = p.text }, c = c }
  end
  return {
...
local function build(p, transparent)
  ...
  -- Floats/popups (LSP hover, Pmenu, Telescope, which-key, Noice cmdline,
  -- blink.cmp, trouble, …) render flush against the editor background --
  -- the same treatment snacks.nvim and neo-tree already get below. This is
  -- the template standard, not a per-plugin exception, so `fl` just tracks
  -- `ed` rather than the elevated surface_container tone.
  local fl = ed
  -- fg can't literally be 'NONE', so tildes always fall back to the real base
  -- colour even when the window itself is transparent.
  local eob = p.base
  ...
  t.TabLineSel = { fg = p.text, bg = p.tab_sel, bold = true }
  ...
  -- snacks.nvim (explorer, file/grep/lsp pickers, etc. all share these).
  -- Flush background, same as every float now (see `fl` above) -- kept
  -- explicit here since Border/Title/Footer default-link down to
  -- FloatBorder/FloatTitle/FloatFooter and not every snacks version honours
  -- that link reliably.
  t.SnacksNormal = { fg = p.text, bg = ed }
  t.SnacksNormalNC = { fg = p.text, bg = ed }
  -- bare fallback: any picker window we don't set explicitly (e.g. preview)
  -- default-links down to this, then to NormalFloat if even this is unset.
  t.SnacksPicker = { fg = p.text, bg = ed }
  t.SnacksPickerBox = { fg = p.text, bg = ed }
  t.SnacksPickerList = { fg = p.text, bg = ed }
  t.SnacksPickerInput = { fg = p.text, bg = ed }
  t.SnacksPickerPreview = { fg = p.text, bg = ed }
  t.SnacksPickerBorder = { fg = p.muted, bg = ed }
  t.SnacksPickerTitle = { fg = p.secondary_soft, bg = ed, bold = true }
  t.SnacksPickerFooter = { fg = p.muted, bg = ed }
  t.SnacksTitle = { fg = p.secondary_soft, bg = ed, bold = true }
  t.SnacksFooter = { fg = p.muted, bg = ed }

  -- neo-tree.nvim
  t.NeoTreeNormal = { fg = p.text, bg = ed }
  t.NeoTreeNormalNC = { fg = p.text, bg = ed }
  t.NeoTreeWinSeparator = { fg = p.muted, bg = ed }
  t.NeoTreeEndOfBuffer = { fg = eob }
...
-- bufferline seeds each filetype's DevIcon highlight (BufferLineDevIconPy,
-- ...Selected, ...) from its own internal tint of Normal, not our palette,
-- and only recomputes that tint on `ColorScheme` -- so anything drawn in
-- between (or before the sweep in repaint_dynamic below catches up) shows
-- the wrong bg, which is what made icon backgrounds look inconsistent with
-- their tab. Patch the one function responsible so every icon highlight it
-- ever creates gets our tab_bg/tab_sel forced onto it immediately, instead
-- of being chased down after the fact. Installed once; reads M.palette live
-- on every call, so it always uses whatever theme is current.
local function patch_bufferline_icons()
  if _G.__matugen_bufferline_icons_patched then
    return
  end
  local ok, hl_mod = pcall(require, 'bufferline.highlights')
  if not ok or type(hl_mod.set_icon_highlight) ~= 'function' then
    return
  end
  local original = hl_mod.set_icon_highlight
  hl_mod.set_icon_highlight = function(state, hls, base_hl)
    local name = original(state, hls, base_hl)
    if name and name ~= '' then
      local cur = vim.api.nvim_get_hl(0, { name = name })
      if cur.fg and M.palette then
        local bg = name:find('Selected', 1, true) and M.palette.tab_sel or M.palette.tab_bg
        vim.api.nvim_set_hl(0, name, { fg = cur.fg, bg = bg })
      end
    end
    return name
  end
  _G.__matugen_bufferline_icons_patched = true
end
...
function M.setup(opts)
  ...
  -- land on grey / white under a light theme; repaint those onto the right
  -- tone. The BufferLineDevIcon sweep below is now a fallback for whatever
  -- patch_bufferline_icons() above missed (a version mismatch, or a group it
  -- ran before the icon even existed) -- the patch is what fixes new icons
  ...
  local function request_repaint(delay)
    ...
    -- one more, longer pass: bufferline's own redraw (which creates the
    -- DevIcon groups we're chasing) isn't always done by `delay`.
    vim.defer_fn(function()
      if mine == repaint_token then
        repaint_dynamic()
      end
  ...
  -- `FileType` matters most here: bufferline creates a buffer's
  -- BufferLineDevIcon<Filetype> group lazily, the first time that filetype's
  -- icon is drawn (also on `ColorScheme`, via bufferline's own cache-reset
  -- autocmd), and always seeds it from its own auto-derived tone rather than
  -- our palette -- this repaint is what pulls it back onto `p.tab_bg`.
  vim.api.nvim_create_autocmd({ 'BufWinEnter', 'WinEnter', 'TabEnter', 'BufAdd', 'FileType', 'TermOpen' }, {
    group = grp,
    callback = function()
      request_repaint(60)
    end,
  ...
  -- The forced redraw above is what makes bufferline actually (re)create its
  -- per-filetype BufferLineDevIcon* groups -- seeded from bufferline's own
  -- tone, not ours. Sweep once more right now, synchronously, so every
  -- currently-open tab's icon is correct immediately rather than waiting for
  -- the scheduled passes above.
  repaint_dynamic()
end
